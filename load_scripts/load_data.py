...
def load_rows_12_14(cur, conn, data, table, spec, fields):
	count = 0
	with open(data) as f:
		rows = f.readlines()
		for row in rows:
...
def parse_row(row, year, spec, fields):
	...
	if int(year) < 2012:
		...
		delta_parsed = row.split(',')
		delta_parsed = delta_parsed[:-1]
	print delta_parsed
	return delta_parsed
...
for year in years['load_years']:
	...
	data_file = '../dat/'+hmda_map['hmda' +year]['data']
	spec_file = '../specs/' + hmda_map['hmda'+year]['spec']
